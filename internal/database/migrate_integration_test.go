...
func TestMigrationsRunAsNonSuperuser(t *testing.T) {
	...
	if applied != 3 || status.Current != 3 || status.Target != 3 || status.Pending {
		t.Fatalf("first Up() = (%+v, %d), want version 3 with three applied migrations", status, applied)
	}
	status, applied, err = migrator.Up(ctx)
	if err != nil {
...
func assertFailedMigrationDoesNotAdvanceVersion(t *testing.T, ctx context.Context, databaseURL string) {
	...
	brokenFS := fstest.MapFS{
		...
		"migrations/00004_broken.sql":  &fstest.MapFile{Data: []byte("-- +goose Up\nSELECT * FROM open_aspm.table_that_does_not_exist;\n")},
	}
	...
	if status.Current != 3 || !status.Pending {
		t.Fatalf("status after failed migration = %+v, want current 3 and pending", status)
	}
}
